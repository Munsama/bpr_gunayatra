...
@section ('head')
    <title>Admin BPR Guna Yatra - Multiguna </title>
...
@section ('konten')
    ...
    <div class="container-fluid">
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">Pinjaman Multiguna</h1>
        <p class="mb-4">Halaman ini berisi data calon nasabah yang mengajukan kredit multiguna. 
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
             <h6 class="m-0 font-weight-bold text-primary"> Data Pengajuan Multiguna</h6>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover" id="multiguna_table" width="100%">
                     <thead>
                      <tr>
                       <th> No </th>
                       <th> Nama</th>
...
$(document).ready(function(){
 ...
 $('#multiguna_table').DataTable({
  processing: true,
  serverSide: true,
  ajax:{
   url: "{{ route('crud_multiguna.index') }}",
  },
  columns:[
 ...
 $('#sample_form').on('submit', function(event){
  ...
  {
   $.ajax({
    url:"{{ route('crud_multiguna.update') }}",
    method:"POST",
    ...
    success:function(data)
    {
     ...
     if(data.success)
     {
      ...
      $('#multiguna_table').DataTable().ajax.reload();
     }
 ...
 $(document).on('click', '.delete', function(){
  multiguna_id = $(this).attr('id');
  $('#confirmModal').modal('show');
 });

 $('#ok_button').click(function(){
  $.ajax({
   url:"crud_multiguna/destroy/"+multiguna_id,
   beforeSend:function(){
