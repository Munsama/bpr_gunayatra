...
@section('head')
<title> BPR Guna Yatra - Berita </title>
<!-- Bootstrap core CSS -->
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-4 blog-entries">
                <!-- Blog Post -->
                <div class="card mb-4 ">
                    <img class="card-img-bottom" src="{{asset('images').('/').($d->foto)}}" />
                    <div class="card-body">
                        <div class="by-post">
                            Diposting pada tanggal {{\Carbon\Carbon::parse($d->tanggal)->format('d M Y')}}
                        </div>
                        <h2 class="card-title">{{$d->judul}}</h2>
                        <p class="card-text">
                            {!!\Illuminate\Support\Str::limit(htmlspecialchars_decode($d->deskripsi),250,"...")!!}</p>
                        <a href="berita/detail_berita/{{$d->id}}" class="btn btn-primary">Continue &rarr;</a>
                    </div>
                </div>
            </div>
            @endforeach
